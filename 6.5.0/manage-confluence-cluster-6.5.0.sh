...
#
####################################################################################
# keep in sync with 'manage-confluence-cluster-6.5.0-version.txt'
...
CONFLUENCE_VERSION="6.5.0"
CONFLUENCE_VERSION_DOT_FREE="650"
CONFLUENCE_LB_PUBLIC_PORT=50650
...
function update_check {
    ...
    local latest_version=$(curl -s https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-data-center/master/6.5.0/manage-confluence-cluster-6.5.0-version.txt?r=${unique_hash})
...
echo -e $C_MGN'  https://github.com/codeclou/docker-atlassian-confluence-data-center/tree/master/6.5.0'$C_RST
...
then
    ...
    echo ""
    docker ps --format '{{.ID}}\t {{.Names}}\t {{.Ports}}' --filter "name=confluence-cluster-650-*"
    echo ""
fi
