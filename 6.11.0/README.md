...
To start, scale, stop and destroy the cluster, the [`manage-confluence-cluster-6.11.0.sh`](https://github.com/codeclou/docker-atlassian-confluence-data-center/blob/master/6.11.0/manage-confluence-cluster-6.11.0.sh) script is provided.
It basically works in the following way:

  * It creates a docker bridge network called `confluence-cluster-6110`.
  * It creates a shared volume called `confluence-shared-home-6110`.
  * It creates a docker container called `confluence-cluster-6110-lb` as the loadbalancer.
    * Binds Port 26110/tcp to the host.
  * It creates a docker container called `confluence-cluster-6110-db` as the database.
    * Port 5432/tcp are only exposed inside the `confluence-cluster-6110` network.
  * It creates one docker containers called `confluence-cluster-6110-node1` as Confluence instance.
  ...
  * On Scale-out: It creates multiple docker containers called `confluence-cluster-6110-node{n}` as Confluence instances.
    * With `n ∈ {2,3,4,...,100}`.
    ...
    * Ports 8090,8091/tcp and the multicast Ports 25500,5701/tcp are only exposed inside the `confluence-cluster-6110` network.
...
The script is meant to follow the convention over configuration paradigma, therefore there is not much to be configured, except two things:
  ...
  * It relies on the hostname `confluence-cluster-6110-lb` pointing to the interface which binds 26110/tcp.
  * It relies on the env variable `CONFLUENCE_DATA_CENTER_LICENSE_6110` to contain a valid Confluence Data Center license.
...
```bash
sudo su
echo "127.0.0.1  confluence-cluster-6110-lb" >> /etc/hosts
```
...
#
curl -so /usr/local/bin/manage-confluence-cluster-6.11.0.sh \
"https://raw.githubusercontent.com/codeclou/docker-atlassian-\
confluence-data-center/master/6.11.0/manage-confluence-cluster-6.11.0.sh"

#
# MAKE EXECUTABLE
#
chmod +x /usr/local/bin/manage-confluence-cluster-6.11.0.sh
```
...
**(1) Start a Confluence® Data Center 6.11.0 Cluster**

```bash
manage-confluence-cluster-6.11.0.sh --action create --scale 1
```
 ...
 * Open a browser to [http://confluence-cluster-6110-lb:26110/](http://confluence-cluster-6110-lb:26110/)
...
```bash
manage-confluence-cluster-6.11.0.sh --action info
```

Should show something like so. If `confluence-cluster-6110-node1` does not show up, then re-run the create cluster command!

```
373f159568f4    confluence-cluster-6110-lb      0.0.0.0:26110->26110/tcp
816c1c17a821    confluence-cluster-6110-node1   5701/tcp, 5801/tcp, 8090-8091/tcp, 25500/tcp
38575aee0e9d    confluence-cluster-6110-db      5432/tcp
```

You can check the logs of each container by calling e.g.:

```bash
docker logs confluence-cluster-6110-node1
```
...
Go to **[http://confluence-cluster-6110-lb:26110/](http://confluence-cluster-6110-lb:26110/)** and make sure you enabled cookies in your Browser (sticky session).
...
Use `confluence-cluster-6110-db` as Servername and `confluence` as username and password.
...
```bash
manage-confluence-cluster-6.11.0.sh --action update --scale 3
```
...
And finally check the **Synchrony Heartbeat**, therefore open [http://confluence-cluster-6110-lb:26110/synchrony/heartbeat](http://confluence-cluster-6110-lb:26110/synchrony/heartbeat) and it should output `ok`.
...
```bash
manage-confluence-cluster-6.11.0.sh --action restart-node --id 3
```
...
```bash
manage-confluence-cluster-6.11.0.sh --action destroy
```
