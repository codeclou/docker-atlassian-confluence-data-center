...
CONFLUENCE_VERSION="6.1.2"
CONFLUENCE_VERSION_DOT_FREE="612"
CONFLUENCE_LB_PUBLIC_PORT=50612
...
function start_instance_confluencenode {
    ...
    docker run \
        ...
        --env NODE_NUMBER=${1} \
        --env CONFLUENCE_DATA_CENTER_LICENSE_612="$CONFLUENCE_DATA_CENTER_LICENSE_612" \
...
function check_if_license_env_var_is_set {
    if [ -z "$CONFLUENCE_DATA_CENTER_LICENSE_612" ]
    then
        echo -e $C_CYN">> license ............:${C_RST}${C_RED} ERROR${C_RST}     - license env var seems to not be set. Please set CONFLUENCE_DATA_CENTER_LICENSE_612"$C_RST
        exit 1
    else
