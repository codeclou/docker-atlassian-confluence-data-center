...
```bash
sudo su
echo "127.0.0.1  confluence-cluster-7132-lb" >> /etc/hosts
```
...
**(1) Start a Confluence Data Center 7.13.2 Cluster**

```bash
rm -rf /opt/confluence-cluster/7.13.2 | true
mkdir -p /opt/confluence-cluster/7.13.2
mkdir -p /opt/confluence-cluster/7.13.2/confluence-home-shared
mkdir -p /opt/confluence-cluster/7.13.2/confluence-home-node1
mkdir -p /opt/confluence-cluster/7.13.2/confluence-home-node2
# If on linux fix permissions for volume mounts
# sudo chown 2002:2002 /opt/confluence-cluster/7.13.2/confluence-*
```
...
docker-compose -f docker-compose-one-node.yml restart confluence-cluster-7132-lb
```
...
- Open a browser to [http://confluence-cluster-7132-lb:27132/](http://confluence-cluster-7132-lb:27132/)
...
15ed1263c551        loadbalancer:v2                     "/work-private/docke…"   0.0.0.0:27132->27132/tcp   confluence-cluster-7132-lb
2994d0d680ad        atlassian/confluence-server:7.13.2   "/tini -- /entrypoin…"   8080/tcp                 confluence-cluster-7132-node1
572fcaf9f669        postgres:9.6                        "docker-entrypoint.s…"   5432/tcp                 confluence-cluster-7132-db
```
...
Go to **[http://confluence-cluster-7132-lb:27132/](http://confluence-cluster-7132-lb:27132/)** and make sure you enabled cookies (sticky session).
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.13.2/docker-compose-two-nodes.yml"

docker-compose -f docker-compose-two-nodes.yml up -d
docker-compose -f docker-compose-two-nodes.yml restart confluence-cluster-7132-lb
```
...
```bash
cd /opt/confluence-cluster/7.13.2
mkdir -p /opt/confluence-cluster/7.13.2/confluence-home-node3
mkdir -p /opt/confluence-cluster/7.13.2/confluence-home-node4
# If on linux fix permissions for volume mounts
# sudo chown 2002:2002 /opt/confluence-cluster/7.13.2/confluence-*
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.13.2/docker-compose-three-nodes.yml"

docker-compose -f docker-compose-three-nodes.yml up -d
docker-compose -f docker-compose-three-nodes.yml restart confluence-cluster-7132-lb
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.13.2/docker-compose-four-nodes.yml"
...
#...
# DOCKER confluence-cluster-7132 network
