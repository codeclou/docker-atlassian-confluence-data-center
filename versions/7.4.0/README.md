...
```bash
sudo su
echo "127.0.0.1  confluence-cluster-740-lb" >> /etc/hosts
```
...
**(1) Start a Confluence Data Center 7.4.0 Cluster**

```bash
rm -rf /opt/confluence-cluster/7.4.0 | true
mkdir -p /opt/confluence-cluster/7.4.0
mkdir -p /opt/confluence-cluster/7.4.0/confluence-home-shared
mkdir -p /opt/confluence-cluster/7.4.0/confluence-home-node1
mkdir -p /opt/confluence-cluster/7.4.0/confluence-home-node2
# If on linux fix permissions for volume mounts
# sudo chown 2002:2002 /opt/confluence-cluster/7.4.0/confluence-*
```
...
docker-compose -f docker-compose-one-node.yml restart confluence-cluster-740-lb
```
...
- Open a browser to [http://confluence-cluster-740-lb:2740/](http://confluence-cluster-740-lb:2740/)
...
15ed1263c551        loadbalancer:v2                     "/work-private/docke…"   0.0.0.0:2740->2740/tcp   confluence-cluster-740-lb
2994d0d680ad        atlassian/confluence-server:7.4.0   "/tini -- /entrypoin…"   8080/tcp                 confluence-cluster-740-node1
572fcaf9f669        postgres:9.6                        "docker-entrypoint.s…"   5432/tcp                 confluence-cluster-740-db
```
...
Go to **[http://confluence-cluster-740-lb:2740/](http://confluence-cluster-740-lb:2740/)** and make sure you enabled cookies (sticky session).
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.4.0/docker-compose-two-nodes.yml"

docker-compose -f docker-compose-two-nodes.yml up -d
docker-compose -f docker-compose-two-nodes.yml restart confluence-cluster-740-lb
```
...
```bash
cd /opt/confluence-cluster/7.4.0
mkdir -p /opt/confluence-cluster/7.4.0/confluence-home-node3
mkdir -p /opt/confluence-cluster/7.4.0/confluence-home-node4
# If on linux fix permissions for volume mounts
# sudo chown 2002:2002 /opt/confluence-cluster/7.4.0/confluence-*
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.4.0/docker-compose-three-nodes.yml"

docker-compose -f docker-compose-three-nodes.yml up -d
docker-compose -f docker-compose-three-nodes.yml restart confluence-cluster-740-lb
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-\
data-center/master/versions/7.4.0/docker-compose-four-nodes.yml"
...
```
curl -I -s http://confluence-cluster-740-lb:2740 | grep ROUTEID
```
...
#...
# DOCKER confluence-cluster-740 network
