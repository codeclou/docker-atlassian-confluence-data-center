...
To start, scale, stop and destroy the cluster, the [`manage-confluence-cluster-6.10.0.sh`](https://github.com/codeclou/docker-atlassian-confluence-data-center/blob/master/6.10.0/manage-confluence-cluster-6.10.0.sh) script is provided.
It basically works in the following way:
  ...
  * On Scale-out: It creates multiple docker containers called `confluence-cluster-6100-node{n}` as Confluence instances.
    * With `n ∈ {2,3,4,...,100}`.
    ...
    * Ports 8090,8091/tcp and the multicast Ports 25500,5701/tcp are only exposed inside the `confluence-cluster-6100` network.
...
```bash
sudo su
echo "127.0.0.1  confluence-cluster-6100-lb" >> /etc/hosts
```
...
#
curl -so /usr/local/bin/manage-confluence-cluster-6.10.0.sh \
"https://raw.githubusercontent.com/codeclou/docker-atlassian-\
confluence-data-center/master/6.10.0/manage-confluence-cluster-6.10.0.sh"

#
# MAKE EXECUTABLE
#
chmod +x /usr/local/bin/manage-confluence-cluster-6.10.0.sh
```
...
**(1) Start a Confluence® Data Center 6.10.0 Cluster**

```bash
manage-confluence-cluster-6.10.0.sh --action create --scale 1
```
...
```bash
manage-confluence-cluster-6.10.0.sh --action info
```
...
```bash
docker logs confluence-cluster-6100-node1
```
...
Use `confluence-cluster-6100-db` as Servername and `confluence` as username and password.
...
```bash
manage-confluence-cluster-6.10.0.sh --action update --scale 3
```
...
```bash
manage-confluence-cluster-6.10.0.sh --action restart-node --id 3
```
...
```bash
manage-confluence-cluster-6.10.0.sh --action destroy
```
