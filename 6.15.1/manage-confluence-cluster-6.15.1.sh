...
#
####################################################################################
# keep in sync with 'manage-confluence-cluster-6.15.1-version.txt'
...
CONFLUENCE_VERSION_DOT_FREE="6151"
CONFLUENCE_LB_PUBLIC_PORT=26151
...
function update_check {
    ...
    local latest_version=$(curl -s https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-data-center/master/6.15.1/manage-confluence-cluster-6.15.1-version.txt?r=${unique_hash})
...
echo -e $C_MGN'  https://github.com/codeclou/docker-atlassian-confluence-data-center/tree/master/6.15.1'$C_RST
...
then
    ...
    echo ""
    docker ps --format '{{.ID}}\t {{.Names}}\t {{.Ports}}' --filter "name=confluence-cluster-6151-*"
    echo ""
fi
