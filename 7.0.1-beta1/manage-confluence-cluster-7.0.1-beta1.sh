...
#
####################################################################################
# keep in sync with 'manage-confluence-cluster-7.0.1-beta1-version.txt'
...
CONFLUENCE_VERSION="7.0.1-beta1"
CONFLUENCE_VERSION_DOT_FREE="701"
CONFLUENCE_LB_PUBLIC_PORT=2701
...
function update_check {
    ...
    local latest_version=$(curl -s https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-data-center/master/7.0.1-beta1/manage-confluence-cluster-7.0.1-beta1-version.txt?r=${unique_hash})
...
echo -e $C_MGN'  https://github.com/codeclou/docker-atlassian-confluence-data-center/tree/master/7.0.1-beta1'$C_RST
...
then
    ...
    echo ""
    docker ps --format '{{.ID}}\t {{.Names}}\t {{.Ports}}' --filter "name=confluence-cluster-701-*"
    echo ""
fi
