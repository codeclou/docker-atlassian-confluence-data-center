...
#
####################################################################################
# keep in sync with 'manage-confluence-cluster-6.14.0-version.txt'
...
CONFLUENCE_VERSION="6.14.0"
CONFLUENCE_VERSION_DOT_FREE="6140"
CONFLUENCE_LB_PUBLIC_PORT=26140
...
function update_check {
    ...
    local latest_version=$(curl -s https://raw.githubusercontent.com/codeclou/docker-atlassian-confluence-data-center/master/6.14.0/manage-confluence-cluster-6.14.0-version.txt?r=${unique_hash})
...
echo -e $C_MGN'  https://github.com/codeclou/docker-atlassian-confluence-data-center/tree/master/6.14.0'$C_RST
...
then
    ...
    echo ""
    docker ps --format '{{.ID}}\t {{.Names}}\t {{.Ports}}' --filter "name=confluence-cluster-6140-*"
    echo ""
fi
